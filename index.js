...
if (typeof(CameraTag) == "undefined") {
  CameraTag = new function() {
    ...
    var webrtc_enabled;
    if (location.protocol === 'https:' && getBrowser()[0] == "Chrome" && getBrowser()[1] > 45) {
      webrtc_enabled = true;
    }
    else if (getBrowser()[0] == "Firefox" && getBrowser()[1] > 39) {
      webrtc_enabled = true;
    }
    else {
    ...
    CT_i18n[46] = CT_i18n[46] || "Campture from Webcam"
    ...
    var publish_asset = function(options) {
      ...
      $.ajax({
        ...
        data: {
          ...
          video_name: options["asset_name"],
          video_description: options["asset_description"]
        },
        success: function(response) {
          if (response["uuid"]) {
  ...
  var publishType = webrtc_enabled ? "webrtc" : "webcam";
  ...
  var setup_interface = function() {
    ...
    webrtc_enabled = webrtc_enabled && $(camera_el).attr("data-webrtc") != "false";
    ...
    bufferSize = $(camera_el).attr("data-buffer-size") || 4096;
  ...
  var populate_hidden_inputs = function() {
    ...
    $(camera.formats).each(function(index, format){
      // videos
      var mp4_url = "//"+appServer+"/download/"+video.uuid+"/"+format.name+"/mp4.mp4";
  ...
  self.reset = function() {
    ...
    error_messages = [];
    readyToPublish = false;
    publishType = webrtc_enabled ? "webrtc" : "webcam";
    if (connected) {
      self.disconnect();
    }
...
// end of CameraTagVideo
  ...
  var perPage = $(wall_el).attr("data-perPage");
  var includeName = $(wall_el).attr("data-includeName");
  var includeDescription = $(wall_el).attr("data-includeDescription");
  ...
  var setup = function(){
    ...
    $.ajax({
      url: "//"+appServer+"/api/v"+CameraTag.version+"/cameras/"+camera_uuid+"/videos/wall.json",
      type: "get",
      data: {
        n: perPage
      },
      success: function(response) {
        if (typeof(response) == "object") {
          ...
          CameraTag.fire(dom_id, "noVideos");
        }
      },
      error: function(jqXHR, textStatus, errorThrown) {
  ...
  var buildInterface = function() {
    ...
    $(videos).each(function(index, video){
      if (video.formats.length > 0) {
        var width = video.formats[0].width;
        var height = video.formats[0].height;
      } else {
        var width = 640;
        var height = 480;
      }
      // create the elements
      var video_el = $('<div id="'+dom_id+'_'+video.uuid+'" class="cameratag_video_container" data-width="'+width+'" data-height="'+height+'"></div>');
      var thumbnail_el = $('<img src="'+video.preview_thumbnail_url+'" class="cameratag_video_thumbnail_container"></img>');
      var name_el = $('<div class="cameratag_video_name">'+video.name+'</div>');
      var description_el = $('<div class="cameratag_video_description">'+video.description+'</div>');

      // assmble it all
      $(video_el).append(thumbnail_el);
      if (includeName) {
        $(video_el).append(name_el);
      }
      if (includeDescription) {
        $(video_el).append(description_el);
      }

      $(container).append(video_el);
      $(video_el).click(function(){
        self.openVideo(video.uuid);
      })
    })
  }
  ...
  self.openVideo = function(uuid) {
    ...
    var max_width = parseInt($("#"+dom_id+"_"+uuid).attr("data-width"));
    var max_height = parseInt($("#"+dom_id+"_"+uuid).attr("data-height"));
    $(lb_body)[0].style.maxWidth = max_width+"px";
    $(lb_body)[0].style.maxHeight = max_height+"px";
    lb_body.html('<video id="new_player" data-options=\'{"width":"100%", "height":"100%"}\' data-uuid="'+uuid+'"></video>');
    lb_overlay.show();
    lb_body.show();
    CameraTag.setup();
  }
...
// End of Video Wall
  ...
  var ImageProcessor = function(placeholder_el) {
    ...
    self.setOffset = function(x,y) {
      ...
      //console.log("x:"+x+" xo:"+x_offset+" xm:"+max_x_offset+" y:"+y+" yo:"+y_offset+" ym:"+max_y_offset);
      console.log("x:"+x+" y:"+y+" pm:"+preview_multiplier+" yo:"+y_offset+" mo:"+max_y_offset);
    }
  ...
  var buildInterface = function() {
    ...
    // container
    container = $('<div class="cameratag_photobooth_contaner"></div>');
    ...
    if (review_screen.length == 0) {
      ...
      apply_button = $('<button class="cameratag_photobooth_apply cameratag_button">APPLY</button>');
      ...
      accept_button = $('<button class="cameratag_photobooth_accept cameratag_button">SAVE</button>');
      retake_button = $('<button class="cameratag_photobooth_retake cameratag_button">START OVER</button>');
  ...
  var renderImage = function() {
    var new_img = $('<img src="'+asset.preview_thumbnail_url+'"></img>')
